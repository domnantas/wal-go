...
**PowerSync Schema Usage**: When querying or inserting data in the app, **always import table definitions from `@/lib/powersync/AppSchema`** (the SQLite schema), not from `@/db/schema` (the PostgreSQL schema). PowerSync syncs data to a local SQLite database, so all application code must use the PowerSync AppSchema for consistency. The PostgreSQL schema in `@/db/schema` is only for database migrations and backend operations.
