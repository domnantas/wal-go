...
`QsoForm`'s field grid goes `1 → sm:2 → md:3`; callsign spans both at `sm` (`sm:col-span-2 md:col-span-1`); date/time uses `sm:col-span-2` (carries to `md`, spanning two of three). The shared `DialogContent` caps at `max-h-[calc(100svh-2rem)]` with `overflow-y-auto`. Import result tables use `overflow-auto` (both axes). The native `type="time"` input carries `appearance-none`; without it iOS Safari gives it an intrinsic width ignoring `w-full`/`min-w-0`, overflowing the dialog.
