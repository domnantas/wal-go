...
`changed` on `Rule` renders an olive "Atnaujinta" badge before the text, for flagging rules changed since the previous season. No rule is currently flagged — clear it once a change stops being recent.
...
The page reflects the **active season's rule set**. It currently describes the **2026-2027** rules; see [scoring.md](scoring.md) for the per-rule-set scoring source of truth.
...
- Contact WAL square is **required** — the field cannot be empty. DX (foreign) contacts are allowed and score: enter the literal `DX` instead of a WAL square. `DX` is stored as-is and never confirms (no square to swap).
