...
- **State colors**: geolocate icon tinted `--brand-golden` (yellow) while tracking/background, `--destructive` on error, `--muted-foreground` when disabled, `--foreground` when idle. Olive is used instead of `--accent` because that token is a low-chroma surface color nearly identical in lightness to `--foreground`, making active indistinguishable from idle (most visible in light theme). The `geolocate-background` state uses a dot-less mask variant (matches upstream).
- **Disabled mask**: when geolocation is unavailable (permission denied / unsupported), the `geolocate:disabled` icon swaps to the struck-through "location off" mask. Required because the shared `background-image: none` rule strips MapLibre's own disabled icon, so otherwise the enabled shape would keep showing (just muted) and falsely imply geolocation is available.
