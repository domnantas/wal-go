...
- **Achievement backfill**: a **"Pasiekimai"** button per season card, plus one above the list that covers every season, calling `admin.achievements.backfill`. **"Bandymas"** beside it runs the same reconcile inside a rolled-back transaction and only reports the counts. Needed because an ended season passes through no scoring write, so nothing else re-evaluates it after the catalogue changes — see [achievements.md](achievements.md) § Backfill. A real run invalidates the dashboard.
