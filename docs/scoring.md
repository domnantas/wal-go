...
Confirmation is **symmetric** — both sides of a confirmed pair independently show their own doubled value, so a confirmed QSO needs no special "shared" representation. A confirmed QSO is marked with an olive `BadgeCheck` icon next to the score (tooltip explains the doubled points) — the same mark used on the profile recent-QSO table.
