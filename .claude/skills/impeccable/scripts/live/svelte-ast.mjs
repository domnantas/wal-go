...
export function loadSvelteCompiler(appRoot) {
	try {
		...
		if (major < 5) {
			return null; // detached mount() previews are svelte 5 only
		}
		...
		const visit = (node, ctx) => {
			...
			switch (node.type) {
				case "Identifier": {
					...
					touches = true;
					if (kind === "index") {
						return; // the runtime each provides it
					}
					if (kind === "item") {
						lossy = true;
						return;
			...
			if (child.type === "ExpressionTag") {
				...
				for (const attr of child.attributes || []) {
					...
					if (HANDLER_ATTR_RE.test(attr.name)) {
						continue; // functions cannot hydrate
					}
					const parts = Array.isArray(attr.value) ? attr.value : [attr.value];
					...
					for (const part of exprParts) {
						...
						if (parts.length !== 1) {
							continue; // mixed static+dynamic value
						}
						attrSlots.push({
							key: slot.key,
				...
				if (boundNested) {
					nestedUnsupported = true; // nested per-item arrays: no hydration plan yet
				}
