...
* Hermes has added support for `atob` and `btoa` in React Native 0.74\. If you are using earlier versions, you may also need to polyfill `atob` and `btoa` in your `package.json`. Packages to try include `text-encoding` and `base-64`, and you can drop `@bacons/text-decoder`. See our [Troubleshooting Guide](/docs/troubleshooting) for quick fixes.
...
`<JazzExpoProvider />` works with various authentication methods, with PassphraseAuth being the easiest way to get started for development and testing. For authentication details, refer to our [Authentication Overview](/docs/key-features/authentication/overview) guide.
...
If you expect to be performing the same operation multiple times in a row, check to see if you can batch the operations. This is particularly true for list insertions, where the list is re-evaluated during each `push`.
...
for (let i = 1; i <= 25000; i++) {
  // [!code --:2]
  const item = Item.create({ name: `Item ${i}` });
  myItems.$jazz.push(item);

  // [!code ++:5]
  batch.push(Item.create({ name: `Item ${i}` }));
  if (batch.length === 1000) {
    myItems.$jazz.push(...batch);
    batch = [];
  }
}
