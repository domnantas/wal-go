...
const GAME_DUPLICATE_TIME_ZONE = "Europe/Vilnius";
...
function getBatchDuplicateKey(params: InsertParams): string {
	const localDay = format(
		new TZDate(params.qsoAt, GAME_DUPLICATE_TIME_ZONE),
		"yyyy-MM-dd"
	);
	return [
...
function getSameQsoDaySql(qsoAt: Date) {
	return sql`DATE(${qso.qsoAt} AT TIME ZONE ${GAME_DUPLICATE_TIME_ZONE}) = DATE(${formatISO(qsoAt)}::timestamptz AT TIME ZONE ${GAME_DUPLICATE_TIME_ZONE})`;
}
...
function scoreBulkInsert(params: InsertParams[]): ScoreDelta[] {
	// Bulk scoring uses base points only.
...
export const season20262027RuleSet: ScoringRuleSet = {
